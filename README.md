...
The information a knowledge worker needs is almost always already on screen. The cost is the reading. The usual workaround — select, copy, switch tab, paste into a chatbot, switch back — is slow enough that most people skim instead, and skimming is where details get missed. Lens removes that loop.
...
5. The Lens icon appears in the toolbar. Pin it for convenience.

## API key

Lens calls a model API using a key you supply. No key is bundled with the extension or committed to this repository.

1. Obtain a Google AI Studio API key from https://aistudio.google.com/apikey
2. Right-click the Lens toolbar icon and choose **Options**.
...
- Chrome blocks all extensions from reading internal browser pages (`chrome://`, the Web Store). Lens reports this rather than failing silently.
