...
// Key Points bullets may end with a verbatim source phrase in [[...]] markers,
// used for click-to-highlight. Split a bullet into its display text and quote.
export function extractQuote(line) {
  // The marker may sit anywhere in the bullet and the model often appends a
  // stray full stop after it, so match the first [[...]] and drop it from the
  // display text wherever it appears.
  const match = /\[\[(.+?)\]\]/.exec(line || "");
  if (!match) return { text: line || "", quote: null };
  const text = (line || "")
    .replace(match[0], " ")
    .replace(/\s+/g, " ")
    .replace(/\s+([.,;:!?])/g, "$1")
    .trim();
  return { text, quote: match[1].trim() };
}

// Remove [[...]] markers for export, so copied markdown reads cleanly.
export function stripQuotes(markdown) {
  return (markdown || "")
    .replace(/\s*\[\[.+?\]\]/g, "")
    .replace(/ +([.,;:!?])/g, "$1");
}
...
export function buildExportMarkdown({ title, url, sections }) {
  ...
  for (const name of SECTIONS) {
    lines.push(`## ${name}`);
    lines.push(stripQuotes(getSection(sections, name)).trim() || "None found");
    lines.push("");
  }
  return `${lines.join("\n").trim()}\n`;
}
