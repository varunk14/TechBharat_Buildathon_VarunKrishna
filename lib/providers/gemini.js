...
export function buildRequestBody({ systemPrompt, userText, image, maxTokens = 1500 }) {
  const parts = [];
  // The image part comes before the text so the model reads the picture as the
  // subject and the text as the instruction about it.
  if (image) {
...
export async function* streamModel({
  ...
  const response = await fetch(ENDPOINT, {
    ...
    body: JSON.stringify(
      buildRequestBody({ systemPrompt, userText, image, maxTokens })
    ),
    signal,
  });
