...
export function buildRequestBody({
  ...
  // A full multi-turn conversation (follow-up questions) is passed through as
  // is; otherwise build a single user turn from the text and optional image.
  let turns = contents;
  if (!turns) {
    const parts = [];
    // The image part comes before the text so the model reads the picture as
    // the subject and the text as the instruction about it.
    if (image) {
...
export async function* streamModel({
  ...
  const response = await fetch(ENDPOINT, {
    ...
    body: JSON.stringify(
      buildRequestBody({ systemPrompt, userText, image, contents, maxTokens })
    ),
    signal,
  });
  ...
  if (!response.ok) {
    ...
    throw statusToError(response.status, detail);
  }
